...
from datafun_streaming.io.io_utils import read_csv_as_lookup
...
OUTPUT_JSONL: Final[Path] = OUTPUT_DIR / "consumed_sales.jsonl"
...
def log_paths() -> None:
    ...
    log_path(LOG, "OUTPUT_JSONL", OUTPUT_JSONL)
...
def initialize_output() -> RunningStats:
    ...
    if OUTPUT_JSONL.exists():
        OUTPUT_JSONL.unlink()
    LOG.info(f"Output JSONL cleared: {OUTPUT_JSONL.name}")
...
def consume_messages(
    ...
    while consumed_count + skipped_count < MAX_MESSAGES:
        ...
        # Also update the JSONL as usual.
        # Filter the record fields based on the contract fields
        filtered_row = {field: enriched.get(field, "") for field in CONSUMED_FIELDNAMES}
        # Append the row as a single line JSON string (JSON Lines format)
        with open(OUTPUT_JSONL, "a", encoding="utf-8") as f:
            f.write(json.dumps(filtered_row) + "\n")
        LOG.info("Appended record to JSONL storage.")
